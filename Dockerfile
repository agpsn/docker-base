...
RUN set -xe && apk update && apk upgrade --no-cache && apk add --no-cache coreutils curl shadow tzdata xz && mkdir "${APP_DIR}" && useradd -u 1000 -U -d "${CONFIG_DIR}" -s /bin/false agpsn && usermod -G users agpsn
